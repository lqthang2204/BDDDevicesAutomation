...
def step_impl(context, loop_number, action, polling):
	for i in range(int(loop_number)):
		...
		if is_break == False and i == int(loop_number) - 1:
			logger.error(f'Failed when execute action {action}')
			raise Exception(f'Failed when execute action {action}')
		if is_break:
			break
...
def step_impl(context, loop_number, action, polling):
	for i in range(int(loop_number)):
		is_break = common_device().execute_action(context.page_present, action, context.driver, context.wait, context.table,
								   context.dict_save_value, context.device['platformName'], context, is_loop=True)
		if is_break == False and i == int(loop_number) - 1:
			logger.error(f'Failed when execute action {action}')
			raise Exception(f'Failed when execute action {action}')
		if is_break:
